...
class Agent(Module):
    ...
    @vmap
    def learning(self, beliefs_A, outcomes, actions, beliefs_B=None, lr_pA=1., fr_pA=1., lr_pB=1., fr_pB=1., **kwargs):
        agent = self
        if self.learn_A:
            o_vec_seq = jtu.tree_map(lambda o, dim: nn.one_hot(o, dim), outcomes, self.num_obs)
            qA = learning.update_obs_likelihood_dirichlet(self.pA, o_vec_seq, beliefs_A, self.A_dependencies, lr=lr_pA, fr=fr_pA)
        ...
        if self.learn_B:
            ...
            qB = learning.update_state_likelihood_dirichlet(self.pB, beliefs_B, actions_onehot, self.B_dependencies, lr=lr_pB, fr=fr_pB)
    ...
    @vmap
    def infer_states_err(self, observations, past_actions, empirical_prior, qs_hist, mask=None):
        """
        Update approximate posterior over hidden states by solving variational inference problem, given an observation.
        ...
        output, err = inference.update_posterior_states_err(
            A,
            self.B,
            o_vec,
            past_actions,
    ...
    @vmap
    def infer_states_vfe(self, observations, past_actions, empirical_prior, qs_hist, mask=None):
        """
        Update approximate posterior over hidden states by solving variational inference problem, given an observation.
